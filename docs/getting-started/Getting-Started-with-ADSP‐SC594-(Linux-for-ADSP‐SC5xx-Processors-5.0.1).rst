...
========================
The build system is currently supported on host PCs running Ubuntu 22.04 LTS 64-bit. For setting up your host PC, please see the `Setting Up Your Host PC <Setting-Up-Your-Host-PC.html>`_ page.
...
.. code-block:: shell
   ...
   mkdir ~/adsp-sc594-yocto-build
   cd ~/adsp-sc594-yocto-build
   mkdir bin
...
-----------------------
Yocto requires the environment to be configured before building is possible.  A setup-environment script in the adsp-sc594-yocto-build folder contains all the required environment settings for your build target.
...
.. code-block:: shell
   ...
   ls tmp/deploy/sdk
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.host.manifest
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1-host.spdx.tar.zst
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.sh
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.target.manifest
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1-target.spdx.tar.zst
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.testdata.json

The ``adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.sh`` is a self-extracting archive containing the SDK.

Installing the SDK
------------------
Invoke the self-extracting archive.
It will default to installing to ``/opt/adi-distro-glibc/5.0.1`` but gives you the option to select your own install folder during the installation.
For the minimal image on SC598

.. code-block:: shell

   ./adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc594-som-ezkit-toolchain-5.0.1.sh
   Analog Devices Inc Reference Distro (glibc) SDK installer version 5.0.1
   =======================================================================
   Enter target directory for SDK (default: /opt/adi-distro-glibc/5.0.1):
   You are about to install the SDK to "/opt/adi-distro-glibc/5.0.1". Proceed [Y/n]?
   Extracting SDK....................................................................................................................done
   Setting it up...done
   SDK has been successfully set up and is ready to be used.
   Each time you wish to use the SDK in a new shell session, you need to source the environment setup script e.g.
    $ . /opt/adi-distro-glibc/5.0.1/environment-setup-cortexa5t2hf-adi_glibc-linux
...
.. code-block:: shell
   ...
   cd /tftpboot
   /opt/adi-distro-glibc/5.0.1/sysroots/x86_64-adi_glibc_sdk-linux/usr/bin/arm-adi_glibc-linux-gnueabi/arm-adi_glibc-linux-gnueabi-gdb u-boot-spl-sc594-som-ezkit.elf
   ...
...
.. code-block:: shell
   ...
   (gdb) load u-boot-proper-sc594-som-ezkit.elf
...
.. code-block:: shell
   ...
   cp tmp/deploy/images/adsp-sc594-som-ezkit/adsp-sc5xx-minimal-adsp-sc594-som-ezkit.rootfs.jffs2 /tftpboot
...
.. code-block:: shell
   ...
   Speed: 1000, full duplex
   BOOTP broadcast 1
   DHCP client bound to address 192.168.1.113 (90 ms)
   SF: Detected is25lp512 with page size 256 Bytes, erase size 64 KiB, total 64 MiB
   ...
   Using eth0 device
   TFTP from server 192.168.1.116; our IP address is 192.168.1.113
   Filename 'adsp-sc5xx-minimal-adsp-sc594-som-ezkit.rootfs.jffs2'.
   Load address: 0x82000000
   Loading: #################################################################
            #################################################################
            #################################################################
...
Booting the minimal image from OSPI
------------------------------------
Repeat the steps to boot from QSPI, but do:
...
In order to boot Linux via the network, the TFTP server should be setup as indicated in the `Setting Up Your Host PC <Setting-Up-Your-Host-PC.html>`_ page, and a copy of the fitImage should be copied into the ``/tftpboot`` directory.
...
In order to boot Linux via NFS, the NFS server should be setup as indicated in `Setting Up Your Host PC: Configure NFS Server <Setting-Up-Your-Host-PC.html#configure-nfs-server>`_.

The root filesystem should then be copied to ``/romfs``.

.. code-block:: shell

   sudo tar -xf tmp/deploy/images/adsp-sc594-som-ezkit/adsp-sc5xx-full-adsp-sc594-som-ezkit.rootfs.tar.xz -C /romfs
...
.. code-block:: shell
   ...
   sudo mkdir /mnt/boot
   sudo tar -xf tmp/deploy/images/adsp-sc594-som-ezkit/adsp-sc5xx-minimal-adsp-sc594-som-ezkit.rootfs.tar.xz -C /mnt
   sudo cp tmp/deploy/images/adsp-sc594-som-ezkit/fitImage /mnt/boot/
   sudo umount /mnt
...
.. note::
   ...
   You can manually change the default boot method U-Boot is going to use upon the next restart and on. To do this, type ``=> edit bootcmd``, erase the current boot method and type in your preferred one. E.g. ``edit: run spiboot``, erase ``spiboot`` and type ``usbboot`` or another boot method command of preference, and hit 'Enter'. Save the change by typing ``saveenv`` and hit 'Enter'. U-Boot will now boot using USB Mass Storage if left uninterrupted when loaded upon subsequent boots
