...
The ADSP-SC573 EZ-KIT comes equipped with 16 MiB of SPI Flash. There's the option of building the ``adsp-sc5xx-tiny image``, after changing the Libc implementation from the default GNU to musl, to further reduce the image's size. Follow the guide in `Boot from SPI Flash on the SC58x & SC573 <../boot/Booting-Linux-from-SPI-Flash-on-SC58x-&-SC573>`
...
Yocto requires the environment to be configured before building is possible.  A setup-environment script in the sc573-ezkit folder contains all the required environment settings for your build target.
...
.. code-block:: shell
   ...
   ls tmp/deploy/sdk
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc573-ezkit-toolchain-3.1.2.host.manifest
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc573-ezkit-toolchain-3.1.2.sh
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc573-ezkit-toolchain-3.1.2.target.manifest
   adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc573-ezkit-toolchain-3.1.2.testdata.json


The ``adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa5t2hf-neon-adsp-sc573-ezkit-toolchain-3.1.2.sh`` is a self-extracting archive containing the SDK.
...
.. code-block:: shell
   ...
   ./adi-distro-glibc-glibc-x86_64-adsp-sc5xx-minimal-cortexa55-adsp-sc573-ezkit-toolchain-3.1.2.sh
...
.. code-block:: shell
   ...
   cd /tftpboot
   /opt/adi-distro-glibc/3.1.2/sysroots/x86_64-adi_glibc_sdk-linux/usr/bin/arm-adi_glibc-linux-gnueabi/arm-adi_glibc-linux-gnueabi-gdb u-boot-spl-sc573-ezkit.elf
   ...
...
.. code-block:: shell
   ...
   sudo mkdir /mnt/boot
   sudo cp tmp/deploy/images/adsp-sc573-ezkit/adsp-sc5xx-ramdisk-adsp-sc573-ezkit.cpio.xz /mnt/boot/adsp-sc5xx-ramdisk-adsp-sc573-ezkit.cpio.xz.u-boot
...
.. list-table::
   ...
   * - :memo:
     - You can manually change the default boot method U-Boot is going to use upon the next restart and on. To do this, type `=> edit bootcmd`, erase the current boot method and type in your preferred one. E.g. `edit: run spiboot`, erase `spiboot` and type `mmcboot` or another boot method command of preference, and hit 'Enter'. Save the change by typing `saveenv` and hit 'Enter'. U-Boot will now boot using the SD card if left uninterrupted when loaded upon subsequent boots
