...
A new sound card ``UAC2Gadget`` is generated both for playback and capture on
the EZ-KIT board:
...
``device 0``) and play the recorded data via normal playback device (aplay
``card 0``, ``device 1``)

.. shell::

   $arecord -f dat -t wav -D hw:1,0 -c 2 -r 48000 -f S16_LE | aplay -D hw:0,0
...
Then, list the available sound card on host PC, to decide which sound card is
associated with the EZ-KIT board:
...
.. shell::
   ...
   $aplay -D hw:1,0 sample_s16_le.wav
