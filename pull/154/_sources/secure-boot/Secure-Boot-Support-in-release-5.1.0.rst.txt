Secure Boot Support (Release 5.1.0)
=====================================
...
*These steps can be skipped if you have previously followed one of the Getting Started guides, such as :doc:`Getting Started with ADSP‐SC598 (Linux for ADSP‐SC5xx Processors 5.1.0) <../getting-started/Getting-Started-with-ADSP‐SC598-(Linux-for-ADSP‐SC5xx-Processors-5.1.0)>`*.
...
.. code-block:: shell
   ...
   cd tmp/deploy/sdk
   sudo ./adi-security-glibc-x86_64-adsp-sc5xx-minimal-cortexa55-adsp-sc598-som-ezkit-toolchain-5.1.0.sh
   Analog Devices Inc Reference Distro (glibc) SDK installer version 5.1.0
   =======================================================================
   Enter target directory for SDK (default: /opt/adi-security/5.1.0):
   You are about to install the SDK to "/opt/adi-security/5.1.0". Proceed [Y/n]? y
   Extracting SDK.......................................................................................................................done
   Setting it up...done
   SDK has been successfully set up and is ready to be used.
   Each time you wish to use the SDK in a new shell session, you need to source the environment setup script e.g.
    $ . /opt/adi-security/5.1.0/environment-setup-cortexa55-adi_glibc-linux
...
Create a folder to serve the images and copy the build output there as mentioned in :doc:`Getting Started with ADSP SC598 5.1.0 <../getting-started/Getting-Started-with-ADSP‐SC598-(Linux-for-ADSP‐SC5xx-Processors-5.1.0)>`

.. note::

   It's always good practice to erase the contents of this folder before running and/or flashing a new build of U-Boot or Linux.
...
.. code-block:: shell
   ...
   sdk_usr=/opt/adi-security/5.1.0/sysroots/x86_64-adi_glibc_sdk-linux/usr/
...
.. code-block:: shell
   ...
   cd /<folder>
   /opt/adi-security/5.1.0/sysroots/x86_64-adi_glibc_sdk-linux/usr/bin/aarch64-adi_glibc-linux/aarch64-adi_glibc-linux-gdb u-boot-spl-sc598-som-ezkit.elf
   ...
...
In the U-Boot console, set the IP address of the Linux PC that hosts the U-Boot loader files, and on the host PC, set up the HTTP server according to the insturctions in 
:doc:`Getting Started with ADSP-SC598 5.1.0 (Bootstrapping to Linux section) <../getting-started/Getting-Started-with-ADSP‐SC598-(Linux-for-ADSP‐SC5xx-Processors-5.1.0)>`. We will use 
this to program the unsigned LDR files for the first boot, and for programming the signed firmware files later.
...
.. code-block:: shell
   ...
   ##################################################
   Packets received 57, Transfer Successful
   Bytes transferred = 75944 (128a8 hex)
   SF: 262144 bytes @ 0x0 Erased: OK
   device 0 offset 0x0, size 0x40000
   SF: 262144 bytes @ 0x0 Written: OK
   ##################################################
   Packets received 734, Transfer Successful
   Bytes transferred = 1056512 (101f00 hex)
   SF: 1835008 bytes @ 0x40000 Erased: OK
   device 0 offset 0x40000, size 0x1c0000
   SF: 1835008 bytes @ 0x40000 Written: OK
...
Reset the board after setting the boot mode to SPI, and break into U-boot by pressing any key.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
We need to bootstrap to Linux to program both the secure keys and the signed firmware to the board. Follow the instructions 
from :doc:`Getting Started with ADSP-SC598 5.1.0 (Bootstrapping to Linux section) <../getting-started/Getting-Started-with-ADSP‐SC598-(Linux-for-ADSP‐SC5xx-Processors-5.1.0)>`

The username is **root** and the password is **adi**.

Program the Secure Firmware
---------------------------
...
and follow the prompts to program the secure firmware to the SPI, eMMC or USB. For more detailed information about 
each of these steps see :doc:`Getting Started with SC598-5.1.0 <../getting-started/Getting-Started-with-ADSP‐SC598-(Linux-for-ADSP‐SC5xx-Processors-5.1.0)>`

Program the secure boot key
---------------------------
After programming the firmware, we need to program the secure key used for signing the firmware into the OTP, where
the BOOTROM can access it and use it to verify the firmware it lodas.
...
Bravo, You have successfully programmed the secure boot key. You can now restart the board and use it normally.
